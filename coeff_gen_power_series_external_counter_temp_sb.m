...
if(doGCM)
    ...
    sGCM_r_dl=xDelay(sGCM_r,1,'delay_r_conv');
    sGCM_i_dl=xDelay(sGCM_i,1,'delay_i_conv');
    
    truncate=1;
    if(truncate)
        blockTemp = xBlock('Convert', struct('arith_type','Signed','n_bits',bit_width,'bin_pt',bit_width-2,'quantization','Truncate','latency',2,'pipeline','on'));
        blockTemp.bindPort({sGCM_r_dl}, {sGCM_rc});
        blockTemp = xBlock('Convert', struct('arith_type','Signed','n_bits',bit_width,'bin_pt',bit_width-2,'quantization','Truncate','latency',2,'pipeline','on'));
        blockTemp.bindPort({sGCM_i_dl}, {sGCM_ic});
    else
        
        blockTemp = xBlock('Convert', struct('arith_type','Signed','n_bits',bit_width,'bin_pt',bit_width-2,'quantization','Round  (unbiased: Even Values)','latency',2,'pipeline','on'));
        blockTemp.bindPort({sGCM_r_dl}, {sGCM_rc});
        blockTemp = xBlock('Convert', struct('arith_type','Signed','n_bits',bit_width,'bin_pt',bit_width-2,'quantization','Round  (unbiased: Even Values)','latency',2,'pipeline','on'));
        blockTemp.bindPort({sGCM_i_dl}, {sGCM_ic});
    end
