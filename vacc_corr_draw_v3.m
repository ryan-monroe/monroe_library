...
name = ['vacc_ctrl'];
if(mode ~=4)
blockSource='monroe_library/vacc_ctrl_freqacc';
else
    blockSource='monroe_library/vacc_ctrl_freqacc_wb';
end
xBlockThis = xBlock(struct('source', blockSource, 'name', name), ...
...
elseif mode==2 %autocorrelation and kurtosis
    for i=1:nPorts
    ...
    for i=1:nPorts
        ...
        name = ['vacc_data_' num2str(i) ];
        blockType= 'monroe_library/vacc_data_forCorr_pk_v3';
...
elseif mode==3 %autocorrelation
    ...
    for i=1:nPorts
        ...
        name = ['vacc_data_' num2str(i) ];
        blockType= 'monroe_library/vacc_data_forCorr_p_v3';
        ...
        xBlockThis = xBlock(struct('source', blockType, 'name', name), ...
            ...
            'bonus_delay', 2, ...
            'bin_pt', in_bp));
        xBlockThis.bindPort({iDin{i}, sVP2}, ...
            {oDout{i}});
    end
    
    elseif mode==4 %cross-correlating pairs of inputs OR autocorrelation; switchable
    for i=1:nPorts
        iDin{i,1}=xInport(['dinU_' num2str(i-1)]);
        iDin{i,2}=xInport(['dinL_' num2str(i-1)]);
    ...
    for i=1:nPorts
        ...
        name = ['vacc_data_' num2str(i) ];
        blockType= 'monroe_library/vacc_data_forCorr_pow_48';
        ...
        xBlockThis = xBlock(struct('source', blockType, 'name', name), ...
            ...
            'bonus_delay', 2, ...
            'bin_pt', in_bp));
        xBlockThis.bindPort({iDin{i,1},iDin{i,2}, sVC2,sVP2, iMode}, ...
            {oDout{i}});
    end
...
slowAcc=(mode~=1 && mode~=0 && mode ~= 4);

d1=0+17*slowAcc+(mode==4);
sAFE_d1=xDelay(sAccFrameEnd,d1,'sAFE_d1');
sAFE_d2=xDelay(sAFE_d1,1,'sAFE_d2');

sVO_d1=xDelay(sValidOut,d1,'sVO_d1');
