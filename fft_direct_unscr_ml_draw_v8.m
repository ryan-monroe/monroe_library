...
function fft_direct_unscr_ml_draw(FFTSize, FFT_Direct_Size, bram_latency,bit_width,coeff_bit_width, out_bit_width, shift, coeff_step_rate)
...
sSyncCoeffIn =  xDelay(sAddSubSyncOut, 2, 'sync_delay1');
sSyncCoeffIn2 = xDelay(sAddSubSyncOut, 4, 'sync_delay2');

sSyncOut = xDelay(sSyncCoeffIn,6,'sync_delay_out');
oSync.bind(sSyncOut);
...
for(i=1:vec_count_linear)
    if(i<=half)
        ...
        a_in = xDelay(sEven_cc2_delay{i},0,blockNameE);
        b_in = xDelay(sOdd_cc2_delay{i},0,blockNameO);
...
%         b_in = sOdd_cc2_delay{i};
    ...
    blockTemp = xBlock(struct('source', str2func('coeff_gen_dual_draw'), 'name',blockName), {coeff_array(1:2^(coeff_step_rate):end,i), 'Block RAM', coeff_bit_width, coeff_step_rate , 0, 0});
    blockTemp.bindPort({sCoeffSyncInAct}, {sCoeffOut{i}});
