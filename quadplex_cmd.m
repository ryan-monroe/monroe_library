...
function quadplex_cmd(FFTSize, bit_width, register_coeffs, max_distro_size, stage_delay_arr, mux_latency_arr)
...
fft_delay = sum(sum(mux_latency_arr) + sum(stage_delay_arr)) + 2^FFTSize -1;
...
coeff_delay_arr(1) = mux_latency_arr(1);
sel_delay_arr(1) = 0;
for(i = 2:fftStages)
   coeff_delay_arr(i) = mux_latency_arr(i);
   sel_delay_arr(i) =  mux_latency_arr(i-1);
   if(i ~= 0)
...
[sum(stage_delay_arr) 2^(FFTSize-2)-1 sum(mux_latency_arr)]
delay_total = sum(stage_delay_arr) + 2^(FFTSize-2)-1 + sum(mux_latency_arr);
