...
function fft_direct_draw(FFTSize, larger_fft_size, input_bit_width, coeff_bit_width, inter_stage_bit_width, output_bit_width, register_coeffs, delay_input, delay_output, sync_tree_input, optional_delay_arr, coeff_group_arr_in, coeff_stage_step_arr, shift_arr,arch)
% xBlock;
% FFTSize = 4;
% larger_fft_size = 13;
...
elseif((length(inter_stage_bit_width) ~= (FFTSize-1))&& (FFTSize ~=1))
...
elseif(length(coeff_bit_width) ~= FFTSize)
    ...
    throwError(strError);
elseif(length(optional_delay_arr) ~= (FFTSize-1) && (FFTSize ~=1))
    ...
    throwError(strError);
elseif(length(coeff_stage_step_arr) ~= FFTSize && (FFTSize ~=1))
    ...
    throwError(strError);
elseif(length(shift_arr) ~= FFTSize && (FFTSize ~=1))
...
elseif(((min(shift_arr) < 0))|| max(shift_arr) > 2)
    strError = strcat('shift_arr must composed of only 0''s, 1''s and 2''s; shift_arr = ', num2str(shift_arr));
    throwError(strError);
...
for(FFTStage = 1:FFTSize)
    ...
    sData = xBulkDelayr(sStageDataOutNew, 2^FFTSize, delay_arr, blockName);
    ...
    for(i = (1:optional_delay_arr_direct(FFTStage+1)))
        ...
        sData = xBulkDelayr(sData, 2^FFTSize, delay_arr,blockName);
    end
