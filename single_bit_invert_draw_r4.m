...
if(bit_to_invert < 5)
   ...
   sDelayed = xDelay(iDin, 2^bit_to_invert+2^(bit_to_invert+1), 'delay');
   oDout.bind(sDelayed);
else
    invertBits = bitWidth-bit_to_invert-1;
   ...
   blockTemp = xBlock('AddSub', struct('mode', 'Addition', 'latency', 0, 'precision', 'User Defined', 'arith_type', 'Unsigned','n_bits', invertBits, 'bin_pt', 0,  'quantization', 'Truncate', 'overflow', 'Wrap', 'hw_selection', 'Fabric', 'use_behavioral_HDL', 'on'), { sToInvert, sConst1}, {sInverted});
