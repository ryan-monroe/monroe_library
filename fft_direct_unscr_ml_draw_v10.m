...
function fft_direct_unscr_ml_draw_v10(FFTSize, FFT_Direct_Size, bram_latency,bit_width,coeff_bit_width, out_bit_width, shift, coeff_step_rate, updateCoeffs, coeffRange,arch)
...
%mitigate my sync fanout
csh=ceil(sqrt(half));
for i=1:csh
    ind1=(i-1)*csh+1;
    ind2=ind1+csh-1;
    temp_val = {xDelay(iSync,1,['syncFanout_' num2str(i)])};
    sync_fanout(ind1:ind2)=temp_val;    
end

%delay my input to this block, so that signals can get there
%(and to account for the sync delay)
for i=1:length(iDataIn)
    dataIn_dl{i}=xDelay(iDataIn{i},1,['dataInDelay_' num2str(i)]);
end
...
for(i=1:2:(half))
    ...
    xlsub2_double_delay_bram_fast.bindPort({dataIn_dl{i}, dataIn_dl{i+1}}, ...
...
for(i=(half+1):2:vec_count_linear)
    ...
    xlsub2_order_rev = xBlock(struct('source', 'monroe_library/order_reverse_2in', 'name', name), ...
        ...
        'bram_latency', bram_latency));
    xlsub2_order_rev.bindPort({sync_fanout{i-half}, dataIn_dl{i}, dataIn_dl{i+1}}, ...
...
blockTemp = xBlock(struct('source', 'monroe_library/delay_bram_fast', 'name', 'dbf_bypass'), ...
    ...
    {dataIn_dl{bypass_index}},{sBypassOut});
...
sSyncCoeffIn =  xDelay(sAddSubSyncOut, 4-1, 'sync_delay1');
sSyncCoeffIn2 = xDelay(sAddSubSyncOut, 5-1, 'sync_delay2');
    
    
    for i=1:csh
    ind1=(i-1)*csh+1;
    ind2=ind1+csh-1;
    temp_val = {xDelay(sSyncCoeffIn,1,['syncCoeff1Fanout_' num2str(i)])};
    sync_coeff1_fanout(ind1:ind2)=temp_val;    
    end
    ...
    ind1=(i-1)*csh+1;
    ind2=ind1+csh-1;
    temp_val = {xDelay(sSyncCoeffIn2,1,['syncCoeff2Fanout_' num2str(i)])};
    sync_coeff2_fanout(ind1:ind2)=temp_val;    
    end
...
sSyncOut = xDelay(sSyncCoeffIn,4+1+2+1,'sync_delay_out');%+3 for rounding
oSync.bind(sSyncOut);
...
for(i=1:vec_count_linear)
    ...
    if(i<=(vec_count_linear/2))
        sCoeffSyncInAct = sync_coeff1_fanout{i};
    else
        sCoeffSyncInAct = sync_coeff2_fanout{i-half};
    end
    sCoeffOut{i} = xSignal;
    if(updateCoeffs)
