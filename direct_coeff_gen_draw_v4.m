...
function direct_coeff_gen_draw_v4(FFTSize, larger_FFTSize, bit_width, register_coeffs, delay_input, sync_tree_input, optional_delay_arr, coeff_group_arr_in, coeff_stage_step_arr, radix4, stage_hires)
...
%         end
        ...
        if( coeffMadeTracker(stageNum, groupNum)==0) %if this coeff generation group does not yet exist...
            ...
            elseif(~radix4)%then we are in full radix2 mode and shouldn't really worry about this bitreverse
                ...
                coeff_in= direct_coeff_gen_calc(FFTSize, fft_stageNum, larger_FFTSize, fft_start_stage, bit_reverse(butterflyNum-1,FFTSize-1));
                stageEffective=fft_stageNum;
