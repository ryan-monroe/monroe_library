...
function quadplex_stage_n_draw(FFTSize, FFTStage, input_bit_width, coeff_bit_width, output_bit_width, bram_latency, shift, mux_latency,stage_mid_delay, arch,muxsel_delay)
%
% xBlock;
% FFTSize=6;
% FFTStage=2;
...
bDC =  xBlock(struct('source', @quadplex_delay_commutator, 'name', blockName), ...
    {vectorLen,bram_latency,mux_latency,input_bit_width,muxsel_delay}, ...
    [{iSync} in {iMuxSel}], ...
...
for i=1:5
    butterfly_in{i}=xDelay(butterfly_in{i}, stage_mid_delay, ['delay_' num2str(i)]);
end
...
if(FFTStage==1)
    ...
    blockTemp = xBlock(struct('source', 'monroe_library/butterfly_radix4_lw', 'name', blockName), ...
        struct('data_width', input_bit_width, 'data_bp', input_bit_width-1, 'coeff_width', coeff_bit_width, 'coeff_bp', coeff_bit_width-2, 'out_bits', output_bit_width,'scale',shift));
