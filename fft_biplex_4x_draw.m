...
function fft_biplex_4x_draw(FFTSize,coeff_bit_width,input_bit_width,inter_stage_bit_width, output_bit_width, shift_arr,max_distro_size_coeff,max_distro_size_delay,register_coeffs,stage_pre_delay,stage_mid_delay,stage_post_delay,mux_latency_arr, bram_latency_fft, bram_latency_unscr,unscr_din_latency, real_inputs, share_coeffs,arch)
% %
%
% xBlock;
% 
...
for(i=1:FFTSize)
    ...
    if(i==1)
        ...
        blockTemp = xBlock(struct('source', str2func('biplex_stage_n_draw'), 'name',blockName), {FFTSize, i, input_bit_width, coeff_bit_width(i), inter_stage_bit_width(i), delays_bram, bram_latency_fft, shift_arr(i), optional_delay_arr(i,:), mux_latency_arr(i), arch});
    ...
    elseif (i==2)
        ...
        blockName = strcat('stage_',num2str(i));
        blockTemp = xBlock(struct('source', str2func('biplex_stage_n_draw'), 'name',blockName), {FFTSize, i, inter_stage_bit_width(i-1), coeff_bit_width(i), inter_stage_bit_width(i), delays_bram, bram_latency_fft, shift_arr(i), optional_delay_arr(i,:), mux_latency_arr(i), arch}, ...
        ...
        blockName = strcat('stage_',num2str(i));
        blockTemp = xBlock(struct('source', str2func('biplex_stage_n_draw'), 'name',strcat('stage_',num2str(i))), {FFTSize, i, inter_stage_bit_width(i-1), coeff_bit_width(i-2), inter_stage_bit_width(i), delays_bram, bram_latency_fft, shift_arr(i), optional_delay_arr(i,:), mux_latency_arr(i), arch}, ...
