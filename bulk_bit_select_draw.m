...
for(i = 1:numInputs)
   ...
   name_c2ri=['c2ri_' stri];
    ...
    name_shiftR=['dsp48e1_variable_shift_r_' stri];
       bShiftR = xBlock(struct('source', 'monroe_library/dsp48e1_variable_shift_v2', 'name', name_shiftR), ...
        struct('data_n_bits', width_in, 'out_n_bits' ,  width_out), ...
        [sR(i), {shiftIn}], ...
        sR_shifted(i));

    name_shiftI=['dsp48e1_variable_shift_i_' stri];
       bShiftI = xBlock(struct('source', 'monroe_library/dsp48e1_variable_shift', 'name', name_shiftI), ...
        struct('data_n_bits', width_in, 'out_n_bits' ,  width_out), ...
        [sI(i), {shiftIn}], ...
        sI_shifted(i));
