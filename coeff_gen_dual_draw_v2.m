...
function coeff_gen_dual_draw(coeff, memory_type, bit_width, bin_pt, step_rate, register_output, mode2, updateCoeffs)
...
sConstVal_0 = xConstVal(0, 'Signed', bit_width, bin_pt, 'Constant3');
...
sCoeff = xDelayr(sCoeffsConcated, register_output, 'Delay');
oCoeff.bind(sCoeff);
...
sBramWriteAddr=xSignal;


if(strcmp(memory_type,'Block RAM'))
if(updateCoeffs==1)
    blockMux = xBlock(struct('source', 'Mux', 'name', ['mux_addr']), ...
        struct('latency', 0), ...
        {iWe, sAddrImag, iAddr}, ...
        {sBramWriteAddr});

    xlsub2_Dual_Port_RAM4.bindPort({sAddrReal, sConstVal_0, sConstBool_0, sBramWriteAddr, iDin, iWe}, ...
