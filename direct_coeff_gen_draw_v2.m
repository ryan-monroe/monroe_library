...
function direct_coeff_gen_draw_v2(FFTSize, larger_FFTSize, bit_width, register_coeffs, delay_input, sync_tree_input, optional_delay_arr, coeff_group_arr_in, coeff_stage_step_arr, radix4)
...
if(radix4)
    for i=1:FFTSize
        if(FFTSize-(2*i-1)+1)>1
            stage_delay_arr(i)=stage_delay_arr(i)+2;
        end
    end
...
while(fft_stageNum<=FFTSize)
    ...
    blockName = strcat('bulk_delay_', num2str(stageNum), '_2');
        ...
        'name', blockName), ...
        {nButterflies,coeff_delay_arr(stageNum)}, sSyncDelayInput, sSyncStageIn);
    ...
    for(butterflyNum = 1:2^(FFTSize-1-doingR4))
        ...
        groupNum = coeff_group_arr(fft_stageNum, butterflyNumEffective);
        if( coeffMadeTracker(stageNum, groupNum)==0) %if this coeff generation group does not yet exist...
            ...
            if(doingR4)
                coeff = direct_coeff_gen_calc(FFTSize, fft_stageNum+1, larger_FFTSize, fft_start_stage, twiddleInds_r4(butterflyNum));
            ...
            if(doingR4)
                ...
                if(radix4 && stageNum==FFTSize)
                    bCoeffGenDual.bindPort(sCoeffSyncStage(floor(groupNum/2)),{xSignal});
                else
                    
                bCoeffGenDual.bindPort(sSyncStageIn(butterflyNum),{xSignal});
                end
...
%         if(doingR4 && (FFTSize-stageNum==2))
%         sCoeffSyncStage_stripped(end+1:2*end)=sCoeffSyncStage_stripped;
%     end
    ...
    if(doingR4 && stageNum>1)
        %         sTemp2(1:2:2*length(sTemp))=sTemp;
        %         sTemp2(2:2:2*length(sTemp))=sTemp;
        sTemp2(1:length(sTemp))=sTemp;
        sTemp2((length(sTemp)+1):2*length(sTemp))=sTemp;
        
        sTemp=sTemp2;
        
    end 
%     if(doingR4 && (FFTSize-stageNum==2))
%         sTemp(end+1:2*end)=sTemp;
%     end
    blockName = strcat('bulk_delay_', num2str(stageNum), '_2');
    bBulkDelay = xBlock(struct('source',str2func('bulk_delay_draw'), ...
        'name', blockName), ...
        {length(sTemp),coeff_delay_arr(stageNum)}, sTemp, sCoeffSyncStage_stripped);
