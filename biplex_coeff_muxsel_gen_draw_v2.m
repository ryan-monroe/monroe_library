...
function biplex_coeff_muxsel_gen_draw_v2(FFTSize, bit_width, register_coeffs_old, max_distro_size, optional_delay_arr, stage_delay_arr, mux_latency_arr)
...
% 
% FFTSize = 7;
% optional_delay_arr = zeros(FFTSize,3);
% stage_delay_arr = [2,5*ones(1,FFTSize-1)];
% register_coeffs_old = 0;
...
powerSeriesFFTSizeThreshold=14;
doingSpecialCoeffGen=double(FFTSize>=powerSeriesFFTSizeThreshold);

register_coeffs_powerSeries = register_coeffs_old;
register_coeffs_old = register_coeffs_old+6*doingSpecialCoeffGen;
...
if(size(optional_delay_arr,1)<3)
    optional_delay_arr(end+1:3,:)=0;
end

if(length(mux_latency_arr)<3)
    mux_latency_arr(1,end+1:3)=0;
end
 
coeff_delay_arr(1) = optional_delay_arr(1,1) + optional_delay_arr(1,2) + mux_latency_arr(1);
sel_delay_arr(1) = optional_delay_arr(1,1);
for(i = 2:max(FFTSize,3))
...
bCountUp = xBlock(struct('source','Counter', 'name', 'Counter_up'), ...
    ...
    2+register_coeffs_old, 'cnt_by_val', 1, 'arith_type', 'Unsigned', 'n_bits', FFTSize-1, 'bin_pt', 0, 'load_pin', 'off',...
...
for(stageNum = 3:FFTSize)
    ...
    if(stageNum<powerSeriesFFTSizeThreshold) %if true, use the old method
        ...
        blockTemp = xBlock(struct('source', str2func('coeff_gen_dual_external_counter_draw_v2'), 'name',blockName), ...
            {coeff, memory_type, bit_width(stageNum-2),FFTSize-stageNum, register_coeffs_old});
        blockTemp.bindPort({sCoeffCounter}, {sCoeff{stageNum-2}});
    else
        
        blockName = strcat('coeff_sync_late_', num2str(stageNum));
        blockName = strcat('coeff_gen_ps_',num2str(stageNum));
        blockTemp = xBlock(struct('source', str2func('coeff_gen_power_series_external_counter'), 'name',blockName), ...
            {FFTSize, bit_width(stageNum-2),FFTSize-stageNum, register_coeffs_powerSeries, 1});
        blockTemp.bindPort({sCoeffCounter}, {sCoeff{stageNum-2}});
...
delay_total = sum(stage_delay_arr) + sum(sum(optional_delay_arr))+2^(FFTSize-2)-1 + sum(mux_latency_arr(1:FFTSize));
