...
function fft_quadplex_4x_draw(FFTSize,coeff_bit_width,input_bit_width,inter_stage_bit_width, ...
    output_bit_width, shift_arr,max_distro_size_coeff,register_coeffs, mux_latency_arr, bram_latency_fft,...
    bram_latency_unscr,unscr_din_latency, real_inputs, share_coeffs,arch)
...
if(share_coeffs==1)
    ...
    stage_delay_arr = [4, (10*ones(1,(FFTSize/2)-1))];
    blockTemp = xBlock(struct('source', str2func('quadplex_cmd'), 'name','quadplex_muxsel_gen'), ...
        {FFTSize, coeff_bit_width, register_coeffs, max_distro_size_coeff, stage_delay_arr, mux_latency_arr}, ...
...
%     else 
    ...
    blockName = strcat('stage_',num2str(stageNum));
    blockTemp = xBlock(struct('source', str2func('quadplex_stage_n_draw_v2'), 'name',blockName), ...
        {FFTSize, stageNum, ibw, cbw, obw, bram_latency_fft, ...
        shift_arr(stageNum), mux_latency_arr(stageNum), arch});
...
%     end
    ...
    blockTemp = xBlock(struct('source', str2func('quadplex_cplx_unscrambler_draw'), 'name','quadplex_cplx_unscrambler'), ...
        {FFTSize, output_bit_width, mux_latency_arr(end), bram_latency_unscr});
