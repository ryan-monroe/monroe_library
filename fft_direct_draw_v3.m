...
blockTemp = xBlock(struct('source', @direct_coeff_gen_draw_v5, 'name', 'direct_coeff_gen'), ...
...
while(actual_FFTStage <=FFTSize)
    ...
    if((actual_FFTStage==1 ) && FFTSize==5 && radix4)
        %         blehMap=[1 9 17 25];
        %         blehMap2=[blehMap blehMap+4];
        %         map=[blehMap2 blehMap2+1 blehMap2+2 blehMap2+3];
        clear map
        mapp([1,17,9,25,3,19,11,27,5,21,13,29,7,23,15,31,2,18,10,26, 4,20,12,28, 6,22,14,30, 8,24,16,32])=1:32;
        mapp_rs=reshape(mapp,8,[]);
        mapp_perm([1,3,5,7,2,4,6,8],:)=mapp_rs;
        map=mapp_perm(:);
    end
    if((actual_FFTStage==3 ) && FFTSize==5 && radix4)
        %         blehMap=[1 9 17 25];
        %         blehMap2=[blehMap blehMap+4];
        %         map=[blehMap2 blehMap2+1 blehMap2+2 blehMap2+3];
        clear map
        ...
        mapp_sub1=[1,5,2,6];
        mapp_sub2=[mapp_sub1 mapp_sub1+2];
        for i=1:4
            mapp_reo1(mapp_sub2,i)=(1:8)+8*(i-1);
        end
        mapp1=mapp_reo1(:);
        mapp2=[1,  5,  3,  7,  2,  6,  4,  8,  9, 13, 11, 15, 10, 14, 12, 16, 17, 21, 19 23, 18, 22, 20, 24, 25, 29, 27, 31, 26, 30, 28, 32];
        %         map(mapp2)=1:32;%might have to swap order here......
        map=mapp2;
    end
    if((actual_FFTStage==5 ) && FFTSize==5 && radix4)
        clear map
        map([ 1,17,9,25, 5,21,13,29, 3,19,11,27, 7,23,15,31, 2,18,10,26, 6,22,14,30, 4,20,12,28, 8,24,16,32])=1:32;
    end
